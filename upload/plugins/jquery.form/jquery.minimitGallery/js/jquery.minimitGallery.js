...
function mg_init(options) {
    ...
    if (activated.length > 0) {
        for (var i in activated) {
            mg_setState(reference, activated[i], "_init", false);
        }
    } else {
        mg_setState(reference, null, "_init", false);
    }
    // set automatic
...
function mg_automatic(reference, append) {
    ...
    if (time != undefined) {
        ...
        if (inverse) {
            this[reference + "_timeout" + append] = this.setInterval(function () {
                mg_prev(reference);
            }, time);
        } else {
            this[reference + "_timeout" + append] = this.setInterval(function () {
...
function mg_automaticpause(reference, append) {
    ...
    if (time != undefined) {
        clearInterval(this[reference + "_timeout" + append]);
        clearInterval(this[reference + "_timeoutpause" + append]);
        this[reference + "_timeoutpause" + append] = this.setInterval(function () {
            mg_automatic(reference, append);
        }, time);
    }
}
...
function mg_setState(reference, num, append, alsolinked) {
    ...
    if (append == "_init") {
        append = "_click";
    } // transform init append to click to have right target etc..
    //
...
function mg_set_mutiple(reference, target, old_target, prevsteps, nextsteps, less, plus, prevtosteps, nexttosteps, cycle) {
    ...
    if (plus > 0 && less > 0) { // calculate mutiple only if plus and less > 0
...
function mg_initLinks(reference, interactive_click, interactive_hover, interactive_out) {
    ...
    while ($("#" + reference + "-" + num).length != 0) {
        if (interactive_click) {
            ...
            $("#" + reference + "-" + num).live('click', function () {
                ...
                mg_automaticpause(reference, "_click");
                mg_setState(reference, parseFloat($(this).attr('id').slice(half + 1)), "_click");
                return false;
            });
        }
        if (interactive_hover) {
            $("#" + reference + "-" + num).live('mouseenter', function () {
                var half = $(this).attr('id').lastIndexOf("-");
                var reference = $(this).attr('id').slice(0, half);
                mg_automaticpause(reference, "_over");
                mg_setState(reference, parseFloat($(this).attr('id').slice(half + 1)), "_hover");
                return false;
            });
        }
        if (interactive_out) {
            $("#" + reference + "-" + num).live('mouseleave', function () {
                var half = $(this).attr('id').lastIndexOf("-");
                var reference = $(this).attr('id').slice(0, half);
                mg_automaticpause(reference, "_out");
                mg_setState(reference, parseFloat($(this).attr('id').slice(half + 1)), "_out");
                return false;
            });
        }
        num++;
    ...
    $("#" + reference + "-first").live('click', function () {
        var half = $(this).attr('id').lastIndexOf("-");
        mg_automaticpause(reference, "_click");
        mg_setState(reference, 0, "_click");
        return false;
    });
    ...
    $("#" + reference + "-last").live('click', function () {
        var half = $(this).attr('id').lastIndexOf("-");
        mg_automaticpause(reference, "_click");
        mg_setState(reference, $("[id^=" + reference + "-]:not([id*=prev]):not([id*=next]):not([id*=first]):not([id*=last])").length - 1, "_click");
        return false;
    });
}
