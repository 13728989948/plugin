$(function ($) {
    ...
    /* path to the .png file;
     ...
     var mapUrl='http://example.com/map/jquery.map_p_001.png';
    ...
    /* height of the arrow under the 'bubble';
     You can remove the arrow in the CSS file,
     after removal of the arrow set:
     var tooltipArrowHeight=0; */
    var IdListyAdresow = '#addresses';
    /* ID for the list of agents addresses
    ...
    if ($('#map-eu').hasClass('multiple-click')) {
        if (searchLink == '') {
    ...
    if (mapUrl == '') {
        var mapUrl = $('#europe').css('background-image').replace(/"/g, "").replace(/url\(|\)$/ig, "");
    }
    ...
    $(mapImg).load(function () {
        ...
        $('#europe').find('span').hide();
        $('#map-eu,#europe').css({'height': 'auto', 'left': '0', 'margin': '0 auto'});
    }).attr('src', mapUrl);
    $.MapClickedRegion = function (e) {
        if ($('#map-eu').hasClass('multiple-click')) {
            ...
            if ($(e).hasClass('active-region')) {
                ...
                $('#europe').find('.active-region').removeClass('active-region');
                $('#europe').find('.focus').removeClass('focus');
                if ($(e).hasClass('active-region')) {
    ...
    $.MapHoveredRegion = function (e) {
        $('#europe').find('.active-region').children('a').hide();
        $(e).children('a').show();
        $(e).addClass('focus');
    }
