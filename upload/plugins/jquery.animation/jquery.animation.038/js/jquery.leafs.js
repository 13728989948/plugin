(function(a2, aF) {
	var w, ai, l = a2.document, aL = a2.location, bi = a2.jQuery, H = a2.$, aa = {}, a6 = [], s = "1.9.0", aH = a6.concat, ao = a6.push, a4 = a6.slice, aM = a6.indexOf, z = aa.toString, V = aa.hasOwnProperty, aQ = s.trim, bI = function(e, b1) {
		return new bI.fn.init(e, b1, w)
	}, bz = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source, ac = /\S+/g, C = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, bp = /^(?:(<[\w\W]+>)[^>]*|#([\w-]*))$/, a = /^<(\w+)\s*\/?>(?:<\/\1>|)$/, bh = /^[\],:{}\s]*$/, bk = /(?:^|:|,)(?:\s*\[)+/g, bF = /\\(?:["\\\/bfnrt]|u[\da-fA-F]{4})/g, aZ = /"[^"\\\r\n]*"|true|false|null|-?(?:\d+\.|)\d+(?:[eE][+-]?\d+|)/g, bR = /^-ms-/, aV = /-([\da-z])/gi, M = function(e, b1) {
	...
	bI.extend({
		Deferred : function(b2) {
			var b1 = [ [ "resolve", "done", bI.Callbacks("once memory"), "resolved" ], [ "reject", "fail", bI.Callbacks("once memory"), "rejected" ], [ "notify", "progress", bI.Callbacks("memory") ] ], b3 = "pending", b4 = {
				state : function() {
					return b3
				},
				always : function() {
		...
		when : function(b4) {
			var b2 = 0, b6 = a4.call(arguments), e = b6.length, b1 = e !== 1 || (b4 && bI.isFunction(b4.promise)) ? e : 0, b9 = b1 === 1 ? b4 : bI.Deferred(), b3 = function(cb, cc, ca) {
				return function(cd) {
					cc[cb] = this;
	...
	bI.support = (function() {
		...
		bI(function() {
			var cd, cg, cf, ce = "padding:0;margin:0;border:0;display:block;box-sizing:content-box;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;", e = l.getElementsByTagName("body")[0];
			if (!e) {
				return
			}
	...
	bI.extend({
		valHooks : {
			...
			select : {
				get : function(e) {
					var b6, b2, b8 = e.options, b4 = e.selectedIndex, b3 = e.type === "select-one" || b4 < 0, b7 = b3 ? null : [], b5 = b3 ? b4 + 1 : b8.length, b1 = b4 < 0 ? b5 : b3 ? b4 : 0;
					for (; b1 < b5; b1++) {
						b2 = b8[b1];
	...
	bX = {
		get : function(b3, b1) {
			var b4 = bI.prop(b3, b1), e = typeof b4 === "boolean" && b3.getAttribute(b1), b2 = typeof b4 === "boolean" ? bE && bO ? e != null : aq.test(b1) ? b3[bI.camelCase("default-" + b1)] : !!e : b3.getAttributeNode(b1);
	...
	bI.event = {
		...
		trigger : function(b1, b6, b4, cf) {
			var b7, cd, b8, ce, b3, b9, cb, b5 = [ b4 || l ], cc = b1.type || b1, b2 = b1.namespace ? b1.namespace.split(".") : [];
			cd = b8 = b4 = b4 || l;
	...
	(function(c8, cf) {
		var cv, b9, cl, cF, cH, cQ, cR, de, cT, cz, cm, cb, cX, c9, b8, cD, cB, c3 = "sizzle" + -(new Date()), cG = c8.document, db = {}, dc = 0, cY = 0, b3 = cx(), c2 = cx(), cE = cx(), c7 = typeof cf, cL = 1 << 31, c5 = [], c6 = c5.pop, b2 = c5.push, ck = c5.slice, b7 = c5.indexOf || function(dg) {
			...
			filter : {
				...
				CHILD : function(df, di, dh, dj, dg) {
					...
					return dj === 1 && dg === 0 ? function(dm) {
						...
						var dm, dz, du, dy, dv, dq, ds = dl !== e ? "nextSibling" : "previousSibling", dx = dt.parentNode, dp = dk && dt.nodeName.toLowerCase(), dn = !dw && !dk;
						if (dx) {
							if (dl) {
								while (ds) {
									du = dt;
		...
		function cc(dg, df, di, dh, dj, e) {
			...
			return ch(function(dv, ds, dm, du) {
				var dx, dt, dp, dn = [], dw = [], dl = ds.length, dk = dv || cw(df || "*", dm.nodeType ? [ dm ] : dm, []), dq = dg && (dv || !df) ? cV(dk, dn, dg, dm, du) : dk, dr = di ? dj || (dv ? dg : dl || dh) ? [] : ds : dq;
				if (di) {
					di(dq, dr, dm, du)
				}
				if (dh) {
		...
		function c4(dk) {
			var df, di, dg, dj = dk.length, dn = cl.relative[dk[0].type], dp = dn || cl.relative[" "], dh = dn ? 1 : 0, dl = co(function(dq) {
				return dq === df
		...
		function cS(dh, dg) {
			var dj = 0, e = dg.length > 0, di = dh.length > 0, df = function(du, dn, dt, ds, dA) {
				var dp, dq, dv, dz = [], dy = 0, dr = "0", dk = du && [], dw = dA != null, dx = de, dm = du || di && cl.find.TAG("*", dA && dn.parentNode || dn), dl = (dc += dx == null ? 1 : Math.E);
				if (dw) {
					de = dn !== cz && dn;
					b9 = dj
				}
	...
	bI.fn.extend({
		...
		closest : function(b4, b3) {
			var b5, b2 = 0, e = this.length, b1 = [], b6 = y.test(b4) || typeof b4 !== "string" ? bI(b4, b3 || this.context) : 0;
			for (; b2 < e; b2++) {
				b5 = this[b2];
		...
		add : function(e, b1) {
			var b3 = typeof e === "string" ? bI(e, b1) : bI.makeArray(e && e.nodeType ? [ e ] : e), b2 = bI.merge(this.get(), b3);
	...
	function k(b3, e) {
		var b1, b4, b2 = 0, b5 = typeof b3.getElementsByTagName !== "undefined" ? b3.getElementsByTagName(e || "*") : typeof b3.querySelectorAll !== "undefined" ? b3.querySelectorAll(e || "*") : aF;
		if (!b5) {
	...
	var E, bn, aD, bg = /alpha\([^)]*\)/i, aT = /opacity\s*=\s*([^)]*)/, bm = /^(top|right|bottom|left)$/, F = /^(none|table(?!-c[ea]).+)/, aY = /^margin/, a9 = new RegExp("^(" + bz + ")(.*)$", "i"), W = new RegExp("^(" + bz + ")(?!px)[a-z%]+$", "i"), S = new RegExp("^([+-])=(" + bz + ")", "i"), bj = {
		BODY : "block"
	}, bb = {
		position : "absolute",
		visibility : "hidden",
		...
		letterSpacing : 0,
		fontWeight : 400
	}, bS = [ "Top", "Right", "Bottom", "Left" ], av = [ "Webkit", "O", "Moz", "ms" ];
	function b(b3, b1) {
		if (b1 in b3) {
			return b1
		}
	...
	function u(b4, b1, e) {
		var b3 = true, b5 = b1 === "width" ? b4.offsetWidth : b4.offsetHeight, b2 = bn(b4), b6 = bI.support.boxSizing && bI.css(b4, "boxSizing", false, b2) === "border-box";
		if (b5 <= 0 || b5 == null) {
			b5 = E(b4, b1, b2);
			if (b5 < 0 || b5 == null) {
				b5 = b4.style[b1]
	...
	if (!bI.support.opacity) {
		bI.cssHooks.opacity = {
			...
			set : function(b4, b5) {
				var b3 = b4.style, b1 = b4.currentStyle, e = bI.isNumeric(b5) ? "alpha(opacity=" + b5 * 100 + ")" : "", b2 = b1 && b1.filter || b3.filter || "";
				b3.zoom = 1;
	...
	bI.extend({
		...
		ajax : function(b5, b2) {
			...
			b2 = b2 || {};
			var ch, b6, cl, b3, ca, ce, b1, cg, b9 = bI.ajaxSetup({}, b2), cn = b9.context || b9, cc = b9.context && (cn.nodeType || cn.jquery) ? bI(cn) : bI.event, cm = bI.Deferred(), cj = bI.Callbacks("once memory"), b7 = b9.statusCode || {}, cd = {}, ck = {}, b4 = 0, b8 = "canceled", cf = {
				readyState : 0,
	...
	var K, ad, bQ = /^(?:toggle|show|hide)$/, bJ = new RegExp("^(?:([+-])=|)(" + bz + ")([a-z%]*)$", "i"), bP = /queueHooks$/, aB = [ h ], a0 = {
		"*" : [ function(e, b7) {
	...
	bI.fn.extend({
		...
		finish : function(e) {
			...
			return this.each(function() {
				var b3, b6 = bI._data(this), b2 = b6[e + "queue"], b1 = b6[e + "queueHooks"], b5 = bI.timers, b4 = b2 ? b2.length : 0;
				b6.finish = true;
				bI.queue(this, e, []);
	...
	bI.each({
		...
		bI.each({
			...
			bI.fn[b3] = function(b7, b6) {
				var b5 = arguments.length && (b2 || typeof b7 !== "boolean"), b4 = b2 || (b7 === true || b6 === true ? "margin" : "border");
	...
	$.AutomLeafStart = function(settings) {
		...
		function Multiply(ID) {
			...
			for (var i = 0; i <= settings.multiplynumber; i++) {
				...
				NewLeaf = '<img id="lf' + leafNum + '" class="AutomLeaf ' + CurrentClass + '" src="' + leafsfolder + "jquery.animation.038_p_00" + Number + '.png"/>';
				$("body").append(NewLeaf);
		...
		function CreateLeafTop(RightLeft) {
			...
			NewLeaf = '<img id="lf' + leafNum + '" class="AutomLeaf x' + SizeRandom + '" src="' + leafsfolder + "jquery.animation.038_p_00" + Number + '.png"/>';
			$("body").append(NewLeaf);
