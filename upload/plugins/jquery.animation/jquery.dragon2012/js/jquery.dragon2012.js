(function ($) {
    $.extend($.dragon, {
        settings: {
            tlTimeBack: 0,// 停留并返回的时间间隔(毫秒)
            hdTimeBack: 0,// 活动并返回的时间间隔(毫秒)
            imagePath: ""// 用于显示的图片路径
        }
    });

    $.fn.dragon = function (settings) {
        var BJ = "getElementsByTagName", A1 = "createElement", BL = "appendChild", A2 = "insertBefore",
            AO = "parentNode", A8 = document, AV = window, Ag = A8.body, BC = Math, Ay = BC.PI, AU = Ay * 2,
            BK = BC.pow, BE = BC.min, Ap = BC.max, Ae = BC.cos, BN = BC.sin, AY = BC.sqrt, Aq = BC.random,
            Ad = parseInt, AJ = /webkit/i
                .test(navigator.userAgent), AG, BD = A8.all, Ai = BD
                && AV.XMLHttpRequest && !!A8[A1]("canvas").getContext, AL = !BD
                || Ai, Ac = A8[BJ]("head")[0], AQ = BA($(this)[0].id), BF = $(this).children()[0], AP = BA("kw"),
            AX = BF[BJ]
            ("img")[0], AZ, Ao = A8[BJ]("area")[0], AN, Al = BA("s_main"), BI = settings.imagePath, g = [
                ["jquery.dragon2012_p_003.png", 83, 83, 1, 1], ["jquery.dragon2012_p_001.png", 28, 28, 1, 8],
                ["jquery.dragon2012_p_002.png", 72, 72, 1, 1], ["jquery.dragon2012_p_005.png", 15, 15, 1, 6],
                ["jquery.dragon2012_p_004.png", 46, 46, 1, 10]], AB = 55, As = [[0, 65],
                [189, 70], [194, 33], [264, 26]], Aw = [10, 25, 11, 26],
            A0 = ".drnfh{filter:fliph;transform:rotateY(180deg);-moz-transform:skew(0,180deg)scale(-1,1);-webkit-transform:rotateY(180deg);-o-transform:skew(0,180deg)scale(-1,1)}.drnb{behavior:url(#default#VML);position:absolute}.drnv{behavior:url(#default#VML);position:absolute;left:0;top:0}.drnp{position:absolute;z-index:100000;left:0;top:0;}",
            AT, AM = [], Ak = [], AI = 1, AD = [
                [[825, 97], [832, 45], [785, 42], [759, 84]],
        ...
        function Ax() {
            ...
            Ab(AZ, "display", "none");
            AZ.src = BI + "jquery.dragon2012_g_002.gif";
            AX.parentNode[BL](AZ);
            w();
            ...
            A9 = setInterval(function () {
                if (AT == "init") {
                    ...
                    if (AT == "run") {
                        ...
                        //返回时间(当AG有定义,且At()-An>2秒,或BG()大于10秒,或Aa为A8的触发事件(比如点击A8(Document))时返回)
                        if ((!AG && At() - An > settings.tlTimeBack) || BG() > settings.hdTimeBack || Aa != A8.activeElement) {
                            AA(AG || At() - An <= settings.tlTimeBack ? 90 : 0)
                        }
                    } else {
                        if (AT == "gohome") {
