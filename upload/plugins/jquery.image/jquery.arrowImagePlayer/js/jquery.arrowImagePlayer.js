...
window.Effect =
    {
        ...
        SwitchTag: function (tit, box, s, show, time) {
            var t = tit.split('/'), b = box.split("/"), ts = $(t[0], t[1]), bs = $(b[0], b[1]), s = s || "onmouseover",
                now = show = show || 0, c;
        ...
        FocusImg: function (time, foc, fbimg, show, fsimg, ftext) {
            var au = $(foc, "a"), bimgs = $(foc, "img[alt!=simg]"), ba = $(fbimg, "dt")[0].find("a")[0], now = 1, tm,
                len = au.length;
        ...
        HtmlMove: function (id, tag, path, rate, upbt, downbt, pgtime, lis) {
            var c, mous = false, fg = tag.split('/'), o = $(id), as = o.find(fg[1]),
        ...
        SelectList: function (id, putid, now) {
            ...
            function isel(i) {
                if (now && now == listi[i].getAttribute("value")) {
                    ...
                    for (var num = 0; num < listi.length; num++) {
